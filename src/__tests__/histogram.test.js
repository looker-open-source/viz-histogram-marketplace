  ...
  beforeEach(() => {
    ...
    queryResponseMock = {
      fields: {
        pivots: [],
        dimension_like: [
          { name: "order_items.id", label: "ID", type: "number" },
        ],
  ...
  describe("done() callback execution in scatterHist & simpleHist", () => {
    ...
    test("simpleHist calls done() when embed promise resolves", async () => {
      ...
      const embedMock = jest
        .fn()
        .mockResolvedValue({ spec: {}, view: mockView });
      const data = [{ order_items_id: 1, order_items_count: 50 }];
      const config = {
        bin_type: "bins",
        max_bins: "10",
        color_col: "#123456",
        color_on_hover: "#654321",
      };
    ...
    test("simpleHist calls done() and registers error when embed promise rejects", async () => {
      const embedMock = jest
        .fn()
        .mockRejectedValue(new Error("Vega embed failure"));
      const data = [{ order_items_id: 1, order_items_count: 50 }];
    ...
    test("scatterHist calls done() and registers error when embed promise rejects", async () => {
      const embedMock = jest
        .fn()
        .mockRejectedValue(new Error("Scatter embed error"));
      const data = [{ order_items_id: 1, order_items_count: 50 }];
      const config = {
        x: "order_items_id",
        y: "order_items_count",
      ...
      queryResponseMock.fields.measure_like.push({
        name: "order_items.amount",
        label: "Amount",
        type: "number",
      });
